...
public class KubernetesServiceImpl implements KubernetesService {
  ...
  private void deploy(DeploymentEntity deploymentEntity) {
    try (KubernetesClient client = new DefaultKubernetesClient(config)) {
      ...
      Deployment createdDeployment = client.apps().deployments().inNamespace(deploymentEntity.getNamespace()).create(deployment);
  ...
  private void updateDeployment(DeploymentEntity deploymentEntity) {
    try (KubernetesClient client = new DefaultKubernetesClient(config)) {
      client.apps().deployments().inNamespace(deploymentEntity.getNamespace()).withName(deploymentEntity.getName())
                    .edit(d -> new DeploymentBuilder(d).editSpec().withReplicas(deploymentEntity.getReplicasCount()).endSpec().build()
                    );
